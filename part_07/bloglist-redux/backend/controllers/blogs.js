...
// POST: add a new comment to an existing blog
blogsRouter.post("/:id/comments", async (request, response, next) => {
  try {
    ...
    // update comment list of the blog
    blog.comments = blog.comments.concat(savedComment._id);
    const updatedBlog = await blog.save();

    // populate the blog to return
    const returnedBlog = await Blog.findById(updatedBlog._id)
      .populate("user", {
        usernameL: 1,
        name: 1,
        id: 1
